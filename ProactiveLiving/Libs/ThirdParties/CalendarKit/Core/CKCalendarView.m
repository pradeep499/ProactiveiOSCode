...
@property (nonatomic, strong) UIButton *listAllButton;
...
-(void)commonInitializer {
    ...
    //[_todayButton setImage:[UIImage imageNamed:@"ic_calendar_today"] forState:UIControlStateNormal];
    [_todayButton setTitle:@"Today" forState:UIControlStateNormal];
    [_todayButton.titleLabel setFont:[UIFont fontWithName:FONT_REGULAR size:14]];
    [_todayButton addTarget:self action:@selector(btnTodayClick:) forControlEvents:UIControlEventTouchUpInside];
    
    //List All button
    _listAllButton =  [UIButton buttonWithType:UIButtonTypeCustom];
    [_listAllButton setTitle:@"List All" forState:UIControlStateNormal];
    [_listAllButton setBackgroundColor:[AppHelper colorFromHexString:@"#00adef" alpha:1.0]];
    [_listAllButton.titleLabel setFont:[UIFont fontWithName:FONT_REGULAR size:16]];
    [_listAllButton addTarget:self action:@selector(btnListAllClick:)forControlEvents:UIControlEventTouchUpInside];
    [AppHelper addShadowOnView:_listAllButton withOffset:CGSizeMake(0.0, 1.0) withColor:[UIColor blackColor]];
...
- (void)layoutSubviewsAnimated:(BOOL)animated
{
    ...
    CGRect headerFrame = CGRectMake(0, 0, width, 64);
    ...
    tableFrame.size.height = CGRectGetHeight(self.superview.frame) - tableFrame.origin.y-48;
    ...
    [_todayButton setFrame:CGRectMake([[self superview] bounds].size.width-50, 35, 50, 13)];
    [[self superview] addSubview:_todayButton];
    
    [_listAllButton setFrame:CGRectMake(SCREEN_WIDTH/2-50, SCREEN_HEIGHT-120, 100, 32)];
    [[self superview] addSubview:_listAllButton];
...
- (IBAction)btnListAllClick:(id)sender {
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"PUSH_ALL_LIST_VC" object:nil];
...
- (void)_layoutCellsAnimated:(BOOL)animated
{
    ...
    for (NSUInteger row = 0; row < rowCount; row++) {
        for (NSUInteger column = 0; column < columnCount; column++) {
            ...
            //cell.dotColor=[UIColor redColor];
            ...
            if([[self dataSource] respondsToSelector:@selector(calendarView:eventsForDate:)])
            {
                ...
                if([[[self dataSource] calendarView:self eventsForDate:workingDate] count]==1)
                {
                    CKCalendarEvent *event=[[[self dataSource] calendarView:self eventsForDate:workingDate]objectAtIndex:0];
                    [cell.dot setBackgroundColor:event.color];
                }
                else
                {
                    cell.dot.backgroundColor=[UIColor whiteColor];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([[self events] count] > 0) {
        if ([[self delegate] respondsToSelector:@selector(calendarView:didSelectDate:withEvents:)]) {
            [[self delegate] calendarView:self didSelectDate:dateToSelect withEvents:[[self dataSource] calendarView:self eventsForDate:dateToSelect]];
        }
    }
}
