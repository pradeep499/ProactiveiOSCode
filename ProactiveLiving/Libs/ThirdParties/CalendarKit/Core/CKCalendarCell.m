...
- (id)init
{
    ...
    if (self) {
        ...
        //  Dot
        _dot = [CalendarCellDot new];
        [_dot setHidden:YES];
        
        // Dot Image
        _dotImage = [UIImageView new];
        [_dotImage setHidden:YES];
...
- (void)setShowDot:(BOOL)showDot
{
    ...
    [[self dotImage] setHidden:!showDot];
}
...
- (void)configureDot
{
    ...
    CGRect dotFrame = CGRectMake(selfWidth - (dotRadius+6),(selfHeight/5) - dotRadius/2, dotRadius, dotRadius);
    [[self dot] setFrame:dotFrame];
    [[self dotImage] setFrame:dotFrame];
