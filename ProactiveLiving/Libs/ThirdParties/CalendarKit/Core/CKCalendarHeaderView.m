...
@property (nonatomic, retain) UIImageView* forwardButtonImage;
@property (nonatomic, retain) UIImageView* backwardButtonImage;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _headerMonthTextFont = [UIFont fontWithName:FONT_BOLD size:14];
        _headerMonthTextColor = kCalendarColorHeaderMonth;
        _headerMonthTextShadow = kCalendarColorHeaderMonthShadow;
        _headerWeekdayTitleFont = [UIFont fontWithName:FONT_REGULAR size:14];
...
- (void)layoutSubviews
{
    ...
    _forwardButtonImage=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ic_calendar_arrowright"]];
    _backwardButtonImage=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ic_calendar_arrowleft"]];

    //Hide actual buttons
    [_forwardButton setHidden:YES];
    [_backwardButton setHidden:YES];
    [_forwardButtonImage setFrame:CGRectMake([self frame].size.width-13-10, 18, 8, 13)];
    [_backwardButtonImage setFrame:CGRectMake(12, 18, 8, 13)];

    [self addSubview:[self forwardButtonImage]];
    [self addSubview:[self backwardButtonImage]];
