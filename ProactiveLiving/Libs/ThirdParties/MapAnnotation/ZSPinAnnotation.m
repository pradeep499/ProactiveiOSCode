...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if(selected)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            self.buttonCustomeCallOut = [UIButton buttonWithType:UIButtonTypeCustom];
            [self.buttonCustomeCallOut addTarget:self action:@selector(buttonHandlerCallOut:) forControlEvents:UIControlEventTouchDown];
            self.buttonCustomeCallOut.tag = 99;
            [self.buttonCustomeCallOut setBackgroundImage:[UIImage imageNamed:@"ic_mapview_calloutpop"] forState:UIControlStateNormal];
            [self.buttonCustomeCallOut setFrame:CGRectMake(-34,-35, 124, 47)];
            
            UILabel *lblTitle = [[UILabel alloc] initWithFrame:CGRectMake(12, 0, 100, 14)];
            lblTitle.text = [self.dicInfo valueForKey:@"name"];
            lblTitle.font = [UIFont boldSystemFontOfSize:10];
            lblTitle.textColor=[UIColor blackColor];
            lblTitle.textAlignment=NSTextAlignmentCenter;
            [self.buttonCustomeCallOut addSubview:lblTitle];
            
            UILabel *lblSubTitle = [[UILabel alloc] initWithFrame:CGRectMake(12, 12, 100, 12)];
            //NSArray *arrServices=[[self.dicInfo valueForKey:@"categoryId"]valueForKey:@"name"];
            //lblSubTitle.text = [NSString stringWithFormat:@"%@",[arrServices componentsJoinedByString:@", "]];
            lblSubTitle.text=[[self.dicInfo valueForKey:@"Validation_Center"]valueForKey:@"name"];
            lblSubTitle.font = [UIFont systemFontOfSize:8];
            lblSubTitle.textColor=[UIColor darkGrayColor];
            lblSubTitle.textAlignment=NSTextAlignmentCenter;
            [self.buttonCustomeCallOut addSubview:lblSubTitle];
            
            ASStarRatingView *viewRating = [[ASStarRatingView alloc] initWithFrame:CGRectMake(38, 20, 48, 14)];
            viewRating.rating = [[self.dicInfo valueForKey:@"rating"]floatValue];
            viewRating.canEdit=NO;
            [self.buttonCustomeCallOut addSubview:viewRating];
