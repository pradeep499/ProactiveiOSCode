...
{
    return  [self.arrMenuTitles count];
}
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([kind isEqual:UICollectionElementKindSectionHeader])
    {
        ...
        if([[self.arrMenuTitles objectAtIndex:indexPath.section ] isEqualToString:@""])
        {
            headerBounds.size.height=0;
            header.frame=headerBounds;
        }
        else
            header.lblHeader.text = [self.arrMenuTitles objectAtIndex:indexPath.section ];
    }
    return header;
}
...
{
    if([[self.arrMenuTitles objectAtIndex:section ] isEqualToString:@""])
